...
uint8_t draw_state = 0; // unassigned variable that helps control the behavior of the display during a variety of settings. 
...
void loop(void) {
  ...
  // Picture do-while loop repeatedly calls to our custom function called 'draw'. 
  // It continues to call until the next page function of our panel returns a value of false or 0.
  ...
  // increase the variable draw_state and we add 1 (++)
  draw_state++;
  if ( draw_state >= 9*8 ) // draw_state will continue to counter upwards until 72.
    draw_state = 0; // when it reaches 72 the counter is reset to 0. 
...
Draw routine calls a function called u8g_prepare() 

void draw(void) {
  u8g_prepare();
  /* Utilzation of a 'Bitwise Shift' where the "<<" operator shifts binary value left.
  While the ">>" operator shifts binary value right. ">> 3" shifts binary value 3 units right.
  Bitshifting Number 8 | 8 = Binary Number 1000 | (8*1)+(4*0)+(2*0)+(1*0) | 8 >> 1 = 4 [0100] | 8 >> 3 = 1 [0001] |  */ 
  switch(draw_state >> 3) { 
...
// Simple function that calls a set of preparatory routines for our panel such as
setting up the font, foreground, etc. 
...
/* u8g.drawBox(10, 12, 20, 30) = means there is a 20x30 pixel box with the left upper corner located at (10, 12) within a 128x64 pixel display. */
void u8g_box_frame(uint8_t a) { // Case 0 = animation of box drifting from left to right on the display. 
...
void u8g_disc_circle(uint8_t a) { // Case 1 = hollow disk 
...
void u8g_r_frame(uint8_t a) { // Case 2 = rectangular box that changes size. 
